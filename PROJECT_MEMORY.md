...
| `a627be8` | **Blog & Blog Archive Heading <br> Removal & Dynamic Tag Support** | Removed hardcoded `<br>` tag separating line 1 and line 2 in `WSS_Blog_Widget` and `WSS_Blog_Archive_Widget` (since `.wss-mask` is already a block element). Added `heading_html_tag` select control (H1–H6, div, span, p) and updated style selectors. |
| *Current* | **Responsive Heading Auto-Scaling System (H1–H6)** | Implemented automatic mobile/tablet font-size scaling for headings (`h1` through `h6`) when only desktop font size is selected. If user explicitly defines a tablet/mobile font size in Elementor (Widget Typography or Site Settings), the explicit value takes full precedence over the auto-scaling rules. |
