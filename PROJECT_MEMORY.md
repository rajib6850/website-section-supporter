...
| `ce37b73` | **Testimonial Widget Style Heading Section Fix** | Added missing `start_controls_section( 'style_heading', ... )` in `WSS_Testimonial_Widget` and audited all 26 widgets to ensure 100% balanced section declaration parity. |
| `bff1abd` | **Home Evaluation Trust Badges / Pills Full Control Suite** | Converted hardcoded trust pills (`100% Confidential & Off-Market`, `Human Econometric Analysis`, `Complimentary 24-48h Delivery`) into full Elementor Repeater controls with custom icons (Shield, Check, Clock, Lock, Star, Award, Custom FontAwesome), toggle switch, and comprehensive styling section (alignment, typography, normal/hover colors, padding, border radius, gap, top spacing). |
| `ee72360` | **Office Location HTML Linebreak & Textarea Support Fix** | Changed `member_address` in `WSS_Team_Widget` to a `TEXTAREA` control and upgraded output rendering with `wp_kses( nl2br( $m_addr ), ... )`. This fixes literal `<br>` escaping so linebreaks and HTML `<br>` tags render as actual line breaks without showing raw HTML tags. Also added line break support to `WSS_CTA_Widget` badge location. |
| `6b39d12` | **Contact Section Layout & Column Ordering Controls** | Added comprehensive Layout & Column Ordering control section to `WSS_Contact_Widget` (`wss_contact`). Supports Desktop Layout (Form on Left / Info on Right or vice versa), Mobile Stack Order (Form on Top / Info on Bottom or vice versa), Tablet Layout (Stacked or Side-by-Side), and responsive column width slider via CSS Grid template areas. |
| *Current* | **Hero Omnibar Native IDX Broker Smart Mode & Unified Style Suite** | Integrated zero-configuration `IDX Broker (Smart Query Formatter)` mode in `WSS_Hero_Widget` (`wss_hero`). Added `idx_results_url` and `idx_open_new_tab` controls. Engineered a dedicated JS query compiler that automatically parses Location (`ccz`), Low & High Price ranges (`lp`/`hp`), Bedrooms (`bd`), Property Types (`pt`), and Status Categories (`a_statusCategory` / Buy / Rent / Sold), preserving luxury noir aesthetics while forwarding searches directly to the client's live MLS/IDX Broker results page. Unified all 4 Elementor Style sections (Search Console, Status Tabs, Fields/Labels/Borders, Search Action Button) so the exact same luxury styling control suite seamlessly styles both Custom URL and IDX Broker modes. Also optimized custom cursor animation loops to prevent idle CPU/GPU consumption. |
