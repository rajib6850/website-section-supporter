...
| *Current* | **About Heading Global Font-Size & Typography Inheritance Fix** | Removed hardcoded CSS `font-size: clamp(...)` rules from `.wss-about-heading, .wss-about h2` in base stylesheet and responsive media queries (`991px`, `768px`, `480px`). Configured `--wss-size-primary: var(--e-global-typography-primary-font-size, inherit);` and `font-size: var(--wss-size-primary, inherit);` so Elementor Global Typography and Site Settings font sizes reflect directly and seamlessly. |
