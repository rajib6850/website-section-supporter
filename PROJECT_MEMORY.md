...
| *Current* | **Contact Section Layout & Column Ordering Controls** | Added comprehensive Layout & Column Ordering control section to `WSS_Contact_Widget` (`wss_contact`). Supports Desktop Layout (Form on Left / Info on Right or vice versa), Mobile Stack Order (Form on Top / Info on Bottom or vice versa), Tablet Layout (Stacked or Side-by-Side), and responsive column width slider via CSS Grid template areas. |
