...
| `9394087` | **About Heading Font-Family Clean Reset for H2 Site Settings** | Completely stripped leftover `font-family: var(--wss-font-primary)` and `font-size` from `.wss-about-heading` and `.wss-about h1-h6` so that Elementor's Site Settings > Theme Style > Typography > H2 applies 100% directly without primary font overrides. |
| *Current* | **Blog & Blog Archive Heading <br> Removal & Dynamic Tag Support** | Removed hardcoded `<br>` tag separating line 1 and line 2 in `WSS_Blog_Widget` and `WSS_Blog_Archive_Widget` (since `.wss-mask` is already a block element). Added `heading_html_tag` select control (H1–H6, div, span, p) and updated style selectors. |
