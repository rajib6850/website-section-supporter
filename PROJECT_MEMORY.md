...
| `6f8704d` | **Universal Elementor Site Settings Theme Style Typography (H1–H6) Architecture** | Refactored all headings (`h1` through `h6`) across all widgets to inherit directly from **Elementor Site Settings > Theme Style > Typography (H1-H6)**. Implemented zero-specificity `:where()` default fallbacks so that if no typography is set in Site Settings, the plugin's default luxury aesthetic applies, while user/theme styles ALWAYS take precedence without specificity conflicts. |
| *Current* | **About Heading Font-Family Clean Reset for H2 Site Settings** | Completely stripped leftover `font-family: var(--wss-font-primary)` and `font-size` from `.wss-about-heading` and `.wss-about h1-h6` so that Elementor's Site Settings > Theme Style > Typography > H2 applies 100% directly without primary font overrides. |
